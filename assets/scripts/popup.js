function updateContent(document, data){
  const update = () => {
    ...
    updateMsg(document, data.minutesToFinish);
    showDate(document);
  }; update();
...
function updateMsg(document, minutesToFinish){
  const msg = document.getElementById('msg');

  if(minutesToFinish >= 0){
    msg.innerHTML = `Faltam apenas <strong>${formatNumber(minutesToFinish/60)} horas</strong> e <strong>${minutesToFinish%60} minutos</strong> para o fim do expediente (6 horas). 🎉`;
  }else{
    msg.innerHTML = `Se preparando para as férias? Você ja fez <strong>${formatNumber((minutesToFinish * (-1))/60)} horas<strong> e <strong>${formatNumber((minutesToFinish* (-1))%60)}</strong> minutos extra.`;
  } 
}
...
function checkConfig(){
  window.localStorage.getItem('config') ? '' : chrome.tabs.create({'url': chrome.runtime.getURL('./config/index.html')}, (tab) => {
    // alert('poera')
  });
}